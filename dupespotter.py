...
general_nuke_regexps = [
	...
	br'(petok|_token|applicationTime)"?:("[-_A-Za-z0-9\.]+"|[0-9\.]+)',
	...
	# id="tab_1-1_340">
	# <a name="tab_1-1_340">
	br'\b(id|name|class)="[^"]{0,100}[-_]\d+"',

	# Randomized anti-spam mailto: lines
	br'<a href="mailto:[^"@]{1,100}@[^"]{2,100}">(&#[0-9a-fA-Fx]{2,4};){3,100}</a>',

	# Kill twitter and facebook share buttons, no matter what kind of
	# URL they stuffed in there.
	br'<div class="fb-like" data-href=".*?</div>',
	br'<a href="https?://twitter.com/share[^"]{0,4096}" class="twitter-share-button.*?</a>',

	# Drupal puts the current URL here, and the casing doesn't always match
	br'<(link rel="(canonical|shortlink|alternate)".{1,1000}?href=|meta property="og:url" content=)"[^"]+" />',

	# Spotted on eff.org drupal
	br'<link href="[^"]+" rel="alternate" hreflang="[^"]+" />',
	...
	br'<input type="hidden"[^>]{1,16384}?>',

	# vbulletin
	br'\(\d+ Viewing\)',
	br'Currently Active Users</a>: \d+ \(\d+ members and \d+ guests\)',

	# v= on http://vstreamers.com/v/images/css/p/videos
	# cb= on megahits.sapo.pt
	# pos= on www.smartcast.com.mx
	br'[&\?]((v|cb)=\d+|pos=[A-Za-z0-9=]+)',

	# spotted on espn.go.com and others
	br'(splinks-|var hash = .|":"?)-?\d+',

	# Kill newrelic inline script
	br'window\.NREUM\|\|\(NREUM=\{\}\);NREUM\.info=\{.{1,3000}?\}',
	...
	br'<(body|div)( id="[^"]+")? class="[^"]+"( class="[^"]+")?( data-src="[^"]{1,2000}")?',
]

drupal_nuke_regexps = [
	# Kill entire Drupal settings line
	br'jQuery\.extend\(Drupal.settings, ?\{.{1,40000}?\}\);',

	# Drupal generates this class id
	br"\bview-dom-id-[0-9a-f]+\b",
	...
	br'<style type="text/css" media="all">@import url\(.{1,4096}?\);</style>',
]

def compile_combined_regexp(patterns):
	regexp = b"|".join(b"(%s)" % p for p in patterns)
...
def process_body(body, url):
	...
	body = re.sub(combined_general_nuke_regexp, b"", body)

	if drupal:
		body = re.sub(combined_drupal_nuke_regexp, b"", body)
